...
class PyTux:
    ...
    def _point_on_track(distance, track, offset=0.0):
        ...
        node_idx = np.searchsorted(
            path_distances[:num_nodes, 1], distance % max_distance) % num_nodes
            ...
            if verbose and not ON_COLAB:
                ...
                if planner:
                    ap = self._point_on_track(
                        kart.distance_down_track + TRACK_OFFSET, track)
                    ax.add_artist(plt.Circle(
                        WH2*(1+aim_point), 2, ec='g', fill=False, lw=1.5))
                plt.pause(1e-3)
            elif verbose and ON_COLAB:
    ...
    for t in track:
        ...
        while n < steps_per_track:
            ...
            steps, how_far = pytux.rollout(
                t, noisy_control, max_frames=1000, verbose=verbose, data_callback=collect)
            print(steps, how_far)
