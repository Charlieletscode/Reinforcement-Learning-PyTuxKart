...
class Planner(torch.nn.Module):
    def __init__(self):
        ...
        # layers.append(torch.nn.Softmax2d())
        
        layers.append(torch.nn.Conv2d(3, 16, 5, 2, 2))
        layers.append(torch.nn.Dropout(p=0.8))
        layers.append(torch.nn.MaxPool2d(3, 1, 1))
        layers.append(torch.nn.AvgPool2d(6, 2, 2))
        layers.append(torch.nn.ReLU())
        layers.append(torch.nn.Conv2d(16, 1, 5, 2, 2))
...
def save_model(model):
    ...
    if isinstance(model, Planner):
        return save(model.state_dict(), path.join(path.dirname(path.abspath(__file__)), 'planner_curr.th'))
