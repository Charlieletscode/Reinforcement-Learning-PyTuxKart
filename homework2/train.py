...
def train(args):
    ...
    print("loading data ...")
    train_data = load_data('drive_data2_dis10_20000', transform=transform,
...
def log(logger, img, label, pred, global_step):
    ...
    ax.add_artist(plt.Circle(
        WH2*(label1[0].cpu().detach().numpy()+1), 2, ec='b', fill=False, lw=1.5))
    ax.add_artist(plt.Circle(
        WH2*(label2[0].cpu().detach().numpy()+1), 2, ec='c', fill=False, lw=1.5))
    ax.add_artist(plt.Circle(
        WH2*(pred1[0].cpu().detach().numpy()+1), 2, ec='r', fill=False, lw=1.5))
    ax.add_artist(plt.Circle(
        WH2*(pred2[0].cpu().detach().numpy()+1), 2, ec='m', fill=False, lw=1.5))
...
if __name__ == '__main__':
    ...
    # parser.add_argument(
    #     '-t', '--transform', default='Compose([ColorJitter(0.2, 0.5, 0.5, 0.2), ToTensor()])')
