...
class SuperTuxDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #     print(len(label))
        #     print(label)
        #     raise Exception("Sorry, got not 4 second2")
        return image, label
...
def show_on_colab():
    ...
    display(ImageSequenceClip(COLAB_IMAGES, fps=15).ipython_display(
        width=512, autoplay=True, loop=True, maxduration=120))
...
class PyTux:
    ...
    def rollout(self, track, controller, planner=None, max_frames=1000, verbose=False, data_callback=None):
        ...
        for t in range(max_frames):
            ...
            extraDistance = 10
            aim_point_world1 = self._point_on_track(
                kart.distance_down_track+TRACK_OFFSET, track)
            aim_point_world2 = self._point_on_track(
                kart.distance_down_track+extraDistance+TRACK_OFFSET, track)
            aim_point_image1 = self._to_image(aim_point_world1, proj, view)
            aim_point_image2 = self._to_image(aim_point_world2, proj, view)
            aim_point1 = aim_point_image1
            aim_point2 = aim_point_image2

            # pdb.set_trace()

            if data_callback is not None:
                data_callback(t, np.array(
                    self.k.render_data[0].image), aim_point_image1, aim_point_image2)  # not sure
            # Rollout calls data_callback(time_step, image, 2d_aim_point) every step, used to store the data

            if planner:
                image = np.array(self.k.render_data[0].image)
                aim_points = planner(TF.to_tensor(image)[None]).squeeze(  # need to changed
                    0).cpu().detach().numpy()
                aim_point1 = aim_points[0:2]
                aim_point2 = aim_points[2:4]

            current_vel = np.linalg.norm(kart.velocity)
            action = controller(aim_point1, aim_point2, current_vel)
            ...
            if verbose and not ON_COLAB:
                ...
                ax.add_artist(plt.Circle(
                    WH2*(1+self._to_image(aim_point_world1, proj, view)), 2, ec='r', fill=False, lw=1.5))
                # new
                ax.add_artist(plt.Circle(
                    WH2*(1+self._to_image(aim_point_world2, proj, view)), 2, ec='m', fill=False, lw=1.5))
                # new
                if planner:
                    ap = self._point_on_track(
                        kart.distance_down_track + TRACK_OFFSET, track)
                    ax.add_artist(plt.Circle(
                        WH2*(1+aim_point1), 2, ec='b', fill=False, lw=1.5))
                    ax.add_artist(plt.Circle(
                        WH2*(1+aim_point2), 2, ec='c', fill=False, lw=1.5))
                plt.pause(1e-3)
            elif verbose and ON_COLAB:
                ...
                p = (aim_point_image1 + 1) * WH2
                draw.ellipse((p[0] - 2, p[1] - 2, p[0] +
                             2, p[1]+2), fill=(255, 0, 0))
                # new
                p = (aim_point_image2 + 1) * WH2
                draw.ellipse((p[0] - 2, p[1] - 2, p[0] +
                             2, p[1]+2), fill=(0, 0, 255))
                # new
                if planner:
                    p = (aim_point1 + 1) * WH2
                    draw.ellipse((p[0] - 2, p[1] - 2, p[0] +
                                 2, p[1]+2), fill=(0, 255, 0))
                    p = (aim_point2 + 1) * WH2
                    draw.ellipse((p[0] - 2, p[1] - 2, p[0] +
                                 2, p[1]+2), fill=(0, 255, 0))
...
def main(pytux, track, output=DATASET_PATH, n_images=10000, steps_per_track=20000, aim_noise=0.1, vel_noise=5, verbose=False, planner=None):
    ...
    for t in track:
        ...
        def collect(_, im, pt1, pt2):
            ...
            if id < images_per_track:
                ...
                with open(fn + '.csv', 'w') as f:
                    f.write('%0.1f,%0.1f' % tuple(pt1))
                    f.write(',%0.1f,%0.1f' % tuple(pt2))  # new
            n += 1
        ...
        while n < steps_per_track:
            def noisy_control(aim_pt1, aim_pt2, vel):
                return control(
                    aim_pt1 + np.random.randn(*aim_pt1.shape) * _aim_noise, aim_pt2 +
                    np.random.randn(*aim_pt2.shape) * _aim_noise,
                    vel + np.random.randn() * _vel_noise)

            steps, how_far = pytux.rollout(
                t, noisy_control, planner, max_frames=1000, verbose=verbose, data_callback=collect)
            print(steps, how_far)
